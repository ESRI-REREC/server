...
const CONFIG = {
  ...
  layerUrl: reqEnv("ARCGIS_LAYER_URL"),
...
/**
 * POST features to the Facilities layer's /addFeatures endpoint using the
 * server-held token. Retries once with a fresh token if the service reports the
 * token stale (498/499). Returns the parsed ArcGIS response.
 */
async function addFeatures(features) {
  const post = async () => {
    ...
    const res = await fetch(`${CONFIG.layerUrl}/addFeatures`, {
      method: "POST",
      headers: {
 ...
 * the form body (e.g. { where, outFields }). Returns the parsed response. */
async function queryLayer(layerUrl, params = {}) {
